...
await showDialog(context: context, builder: (_) => const _AddContactDialog());
await _refresh();
}
...
if (items.isEmpty) return const Center(child: Text('No contacts yet. Tap + to add.'));
...
try {
await SupabaseService().addContact(name: _name.text.trim(), phone: _phone.text.trim(), relation: _relation.text.trim());
...
TextField(controller: _relation, decoration: const InputDecoration(labelText: 'Relation')),
],
),
actions: [
TextButton(onPressed: _saving ? null : () => Navigator.pop(context), child: const Text('Cancel')),
FilledButton(onPressed: _saving ? null : _save, child: _saving ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator()) : const Text('Save')),
],
);
}
}